    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          "https://nike-mock.onrender.com/api/new_products/"
        );
    ...
    <section id="new_arrivals" className="py-10">
      <div className={styles.header}>
        <h2 className="text-4xl font-palanquin font-bold">
          See What's <span className="text-coral-red"> New </span>
        </h2>
        <div className="flex items-center">
          ...
          <Link
            to={"/products?new_arrivals=true"}
            onClick={() =>
              handleFilterChange({
                target: { name: "new_arrivals", value: e.target.checked },
              })
            }
          >
      ...
      <Slider {...settings} ref={sliderRef} className="max-w-7xl mx-auto px-4">
        {products.map((product) => (
          <div key={product._id} className="px-2">
            <ProductCard product={product} showAddToCart={false} label="New " />
          </div>
        ))}
      </Slider>
    </section>
